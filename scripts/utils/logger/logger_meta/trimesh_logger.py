...
class TrimeshLogger(BaseLogger):
    def __init__(self, tb_logger, log_path, config) -> None:
        """
        Initialize the TrimeshLogger object.
    ...
    def log_batch(self, batch) -> None:
        ...
        for mesh_key in keys_list:  # for each key
            ...
            kdata = data[mesh_key]
            if isinstance(kdata, list):  # List of Trimesh objects
                assert all(isinstance(m, trimesh.Trimesh) for m in kdata)
                for i, mesh in enumerate(kdata):
                    viz_id = meta_info["viz_id"][i]

                    # Export .obj mesh files
                    mesh.export(f"{epoch_dir}/{mesh_key}_{viz_id}.obj")
