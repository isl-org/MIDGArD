...
def rpy_to_matrix(coords):
    ...
    """
    coords = np.asanyarray(coords, dtype=np.float32)
...
def load_meshes(filename):
    ...
    try:
        ...
        meshes = [trimesh.creation.icosphere(radius=1e-6)]
...
class Box(URDFType):
    ...
    @size.setter
    def size(self, value):
        self._size = np.asanyarray(value).astype(np.float32)
        self._meshes = []
...
class Material(URDFType):
    ...
    def _from_xml(cls, node, path):
        ...
        if color is not None:
            color = np.fromstring(color.attrib["rgba"], sep=" ", dtype=np.float32)
        kwargs["color"] = color
...
class Inertial(URDFType):
    ...
    def _from_xml(cls, node, path):
        ...
        inertia = np.array([[xx, xy, xz], [xy, yy, yz], [xz, yz, zz]], dtype=np.float32)
...
class Joint(URDFType):
    ...
    def get_child_pose(self, cfg=None):
        ...
        if cfg is None:
            ...
            if cfg is None:
                cfg = np.zeros(2, dtype=np.float32)
            else:
                cfg = np.asanyarray(cfg, dtype=np.float32)
            if cfg.shape != (2,):
            ...
            if cfg is None:
                cfg = np.zeros(6, dtype=np.float32)
            else:
    ...
    @classmethod
    def _parse_and_fix_axis(cls, axis_str):

        tokens = axis_str.attrib["xyz"].split()
        ...
        # If none are invalid or multiple are invalid, do a simple fallback
        if invalid_index is None or tokens.count("None") > 1:
            # Just parse normally or fallback to the earlier "replace with 0.0" approach
            cleaned_tokens = [t if t.lower() != "none" else "0.0" for t in tokens]
            axis_array = np.array([float(x) for x in cleaned_tokens], dtype=float)
        else:
            # Exactly one invalid token => solve for it
            # Identify the valid indices:
            valid_indices = [
                i for i, val in enumerate(numeric_values) if val is not None
            ]
            a = numeric_values[valid_indices[0]]
            b = numeric_values[valid_indices[1]]

            # One token is unknown, so that unknown^2 = 1 - a^2 - b^2
            missing_val_sq = 1.0 - a * a - b * b
            if missing_val_sq < 0:
                # If negative, fallback or clamp to zero
                missing_val = 0.0
            else:
                # Choose sign here:
                # Option: default to positive
                missing_val = np.sqrt(missing_val_sq)
                # Alternatively, default to negative: missing_val = -np.sqrt(missing_val_sq)

            numeric_values[invalid_index] = missing_val
            axis_array = np.array(numeric_values, dtype=float)

        # Finally, re-normalize to be safe, if it is non-zero
    ...
    def _from_xml(cls, node, path):
        ...
        if axis is not None:
            axis = cls._parse_and_fix_axis(axis)
...
class URDF(URDFType):
    ...
    def link_fk_batch(self, cfgs=None, link=None, links=None, use_names=False):
        ...
        for lnk in self._reverse_topo:
            if lnk not in link_set:
                continue
            poses = np.tile(np.eye(4, dtype=np.float32), (n_cfgs, 1, 1))
            path = self._paths_to_base[lnk]
            for i in range(len(path) - 1):
                child = path[i]
                parent = path[i + 1]
    ...
    def animate(self, cfg_trajectory=None, loop_time=3.0, use_collision=False):
        ...
        # If it is specified, parse it and extract the trajectory length.
            ...
            if len(ct) > 0:
                for k in ct:
                    val = np.asanyarray(ct[k]).astype(np.float32)
                    if traj_len is None:
                        traj_len = len(val)
            ...
            ct = np.asanyarray(ct).astype(np.float32)
            if ct.ndim == 1:
                ct = ct.reshape(-1, 1)
    ...
    def _process_cfgs(self, cfgs):
        ...
        if isinstance(cfgs, dict):
            ...
            if isinstance(cfgs[0], dict):
                ...
                cfgs = np.asanyarray(cfgs, dtype=np.float32)
