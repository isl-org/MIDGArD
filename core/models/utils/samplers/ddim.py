...
class DDIMSampler(object):
    ...
    def register_buffer(self, name, attr):
        if type(attr) == torch.Tensor:
            if attr.device != torch.device("cuda"):
                attr = attr.to(torch.device("cuda"), dtype=torch.float32)
    ...
    def sample(
        ...
        **kwargs,
        ...
        samples, intermediates = self.ddim_sampling(
            ...
            part_conditioning=part_conditioning,
    ...
    def ddim_sampling(
        ...
        for i, step in enumerate(iterator):
            ...
            if mask is not None:
                ...
                # Corrupt x0 with ts steps of noise
                img_orig = self.model.q_sample(
                    x0, ts
                )  # TODO: deterministic forward pass?

                # Mask known = img_orig (corrupted) vs unknown = img (pure noise at the beginning)
    ...
    def p_sample_ddim(
        ...
        if part_conditioning is not None:
            x_in = torch.cat([x, part_conditioning], dim=1)
        else:
            x_in = x
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.0:
            ...
            uc_img, uc_txt = (
                unconditional_conditioning["uc_img"],
                unconditional_conditioning["uc_txt"],
            )
            ...
            x_in = torch.cat([x_in] * 3)
            t_in = torch.cat([t] * 3)
            c_in = torch.cat([mm_uc_feat, c_img_with_txt, c_txt_with_img])
            e_t_uncond, e_t_img, e_t_txt = self.model.apply_model(
                x_in, t_in, c_in
            ).chunk(3)
            e_t = (
                e_t_uncond
        ...
        sqrt_one_minus_at = torch.full(
            param_shape, sqrt_one_minus_alphas[index], device=device
        )
        ...
        if quantize_denoised:
            ...
            pred_x0, _, *_ = self.model.vqvae.neural_network.quantize(
                pred_x0, is_voxel=True
            )
