...
def Normalize(in_channels, norm_layer=torch.nn.GroupNorm):
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        # actually bn1, bn2, ..., are groupnorm
        self.bn1 = norm_layer(num_channels=planes)
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.bn1 = norm_layer(num_channels=width)
        self.conv2 = conv3x3(width, width, stride, groups, dilation)

        self.bn2 = norm_layer(num_channels=width)
        self.conv3 = conv1x1(width, planes * self.expansion)

        self.bn3 = norm_layer(num_channels=planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    def __init__(
        ...
        norm_layer: Optional[Callable[..., nn.Module]] = nn.GroupNorm,
    ) -> None:
        ...
        # self.avgpool = nn.AdaptiveAvgPool3d((8, 8, 8))
    ...
    **kwargs: Any,
    ...
    if pretrained:
        ...
        try:
            ...
            cprint("[*] loading state_dict with strict=False!", "cyan")
            model.load_state_dict(state_dict, strict=False)

        cprint("[*] %s successfully loaded from %s." % (arch, model_urls[arch]), "cyan")
    return model
