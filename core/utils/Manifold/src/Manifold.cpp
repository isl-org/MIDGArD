...
// Destructor
// Frees the Octree structure and clears the bounding volume (BV) list to release memory
Manifold::~Manifold()
{
	delete tree;
	tree = nullptr;
...
// Load the geometric data from a given file
// Supports OBJ, OFF, PLY, STL formats
int Manifold::Load(char *filename)
{
	...
	if (extension == "obj")
	{
		igl::readOBJ(filename, V, F); // Load OBJ file
	}
	else if (extension == "off")
	{
		igl::readOFF(filename, V, F); // Load OFF file
	}
	else if (extension == "ply")
	{
		igl::readPLY(filename, V, F); // Load PLY file
	}
	else if (extension == "stl")
	{
		igl::readSTL(filename, V, F, N); // Load STL file
	}
	else
	{
	...
	// Convert geometric data from Eigen structure to GLM structure
...
// Calculate the axis-aligned bounding box for the geometry
void Manifold::Calc_Bounding_Box()
{
	// Initial bounding box with extreme values
	min_corner = glm::dvec3(1e30, 1e30, 1e30);
	max_corner = -min_corner;

	// Update bounding box based on vertex positions
	...
	// Pad the bounding box slightly to avoid numerical issues at the edges
	glm::dvec3 length = max_corner - min_corner;
	min_corner -= length * 0.2;
	max_corner += length * 0.2;
}

// Build a spatial subdivision tree to accelerate geometric queries
// Resolution parameter controls the depth of the subdivision
void Manifold::Build_Tree(int resolution)
{
	Calc_Bounding_Box();
	tree = new Octree(min_corner, max_corner, face_indices, 0.01);
	
	// Subdivide tree until the number of elements reaches `resolution`
	while (tree->number < resolution)
	{
		tree->Split(vertices);
	}

	// Establish connection information in the tree
	tree->BuildConnection();
	tree->BuildEmptyConnection();

	// Determine which octree cells are exterior
	...
	// Process list of exterior cells and neighbors
	while (!empty_list.empty())
	{
		...
		for (auto &neighbor : empty->empty_neighbors)
		{
			if (empty_set.insert(neighbor).second)
			{
				empty_list.push_back(neighbor);
			}
		}
	}
}

// Find the closest point on a triangle to a given source position
// Optimized function to find the closest point on a triangle to a given source position
glm::dvec3 Manifold::Closest_Point(const glm::dvec3 *triangle, const glm::dvec3 &sourcePosition)
{
    const glm::dvec3 edge0 = triangle[1] - triangle[0];
    const glm::dvec3 edge1 = triangle[2] - triangle[0];
    const glm::dvec3 v0 = triangle[0] - sourcePosition;

    const double a = glm::dot(edge0, edge0);
    const double b = glm::dot(edge0, edge1);
    const double c = glm::dot(edge1, edge1);
    const double d = glm::dot(edge0, v0);
    const double e = glm::dot(edge1, v0);

    const double det = a * c - b * b;
    double s, t;

    if (det == 0) {
        // The triangle is degenerate (the points are collinear or coincident)
        return triangle[0];
    }

    const double invDet = 1.0 / det;
    const double sNumer = b * e - c * d;
    const double tNumer = b * d - a * e;

    // If sNumer and tNumer are both negative, s + t is guaranteed to be less than det.
    if (sNumer + tNumer <= det) {
        if (sNumer < 0.0) {
            if (tNumer < 0.0) {
                // Region 4
                s = (d < 0.0) ? -d / a : 0.0;
        ...
        if (sNumer < 0.0) {
            ...
            numer = c + e - b - d;
            denom = a - 2.0 * b + c;
            s = (numer <= 0.0) ? 0.0 : (numer >= denom) ? 1.0 : numer / denom;
            t = 1.0 - s;
        }
    }

    // Clamp s and t to [0, 1] and ensure s + t <= 1
    s = std::max(0.0, std::min(1.0, s));
    t = std::max(0.0, std::min(1.0, t));
    if (s + t > 1.0) {
        const double sum = s + t;
        s /= sum;
        t /= sum;
    }
...
// glm::dvec3 Manifold::Closest_Point_Legacy(const glm::dvec3 *triangle, const glm::dvec3 &sourcePosition)
// {
// 	const glm::dvec3 edge0 = triangle[1] - triangle[0];
// 	const glm::dvec3 edge1 = triangle[2] - triangle[0];
// 	const glm::dvec3 v0 = triangle[0] - sourcePosition;
...
// Construct the manifold, including creating/managing various face and vertex information
void Manifold::Construct_Manifold()
{
	...
	vertices.clear();
	for (size_t i = 0; i < hash_v.size(); ++i)
	{
		if (hash_v[i])
		{
	...
	face_indices = std::move(triangles); // Move instead of copying
}

// Find the closest point on the manifold to a given vertex index `i`
glm::dvec3 Manifold::Find_Closest(int i)
{
	glm::dvec3 cpoint = glm::dvec3(1e20, 1e20, 1e20);
	glm::dvec3 tris[3];
	glm::dvec3 normal;
	for (const int &face_ind : v_faces[i])
	{
		for (int j = 0; j < 3; ++j)
		{
...
// Project the manifold to some intended representation based on properties like sharpness
void Manifold::Project_Manifold()
{
	...
	for (size_t i = 0; i < face_indices.size(); ++i)
	{
		for (int j = 0; j < 3; ++j)
		{
			vertex_faces[face_indices[i][j]].push_back(i);
		}
	}
	...
	for (int iter = 0; iter < ITER_NUM; ++iter)
	{
		...
		for (size_t i = 0; i < face_indices.size(); ++i)
		{
			glm::ivec3 &fid = face_indices[i];
			face_normals[i] = glm::normalize(glm::cross(vertices[fid[1]] - vertices[fid[0]],
														vertices[fid[2]] - vertices[fid[0]]));
		}
		...
		int c = 0;
		for (size_t i = 0; i < invalid_vertices.size(); ++i)
		{
			if (invalid_colors[i] == -1)
			{
				...
				while (f != queue.size())
				{
					...
					for (size_t j = 0; j < vertex_faces[id].size(); ++j)
					{
						for (int k = 0; k < 3; ++k)
						{
...
// Process the entire manifold from its geometric data to a realized structure
void Manifold::Process_Manifold(int resolution)
{
	...
	{
		if (dis_weight[i] > 0)
			vertices[i] = dis[i] / static_cast<double>(dis_weight[i]);
	}
...
bool Manifold::Split_Grid(map<Grid_Index, int> &vcolor, vector<glm::dvec3> &nvertices, vector<glm::ivec4> &nface_indices, vector<set<int>> &v_faces, vector<glm::ivec3> &triangles)
{
	double unit_len = 0;
	v_info.resize(vcolor.size());
	for (const auto &pair : vcolor)
	{
		v_info[pair.second] = pair.first;
	}
	set<int> marked_v;
	map<pair<int, int>, list<pair<int, int>>> edge_info;
	for (size_t i = 0; i < nface_indices.size(); ++i)
	{
		for (int j = 0; j < 4; ++j)
		{
			...
			auto &listRef = edge_info.emplace(edge, std::list<std::pair<int, int>>()).first->second;
			listRef.emplace_back(i, j);
		}
	}
	for (const auto &pair : edge_info)
	{
		if (pair.second.size() > 2)
		{
			marked_v.insert(pair.first.first);
			marked_v.insert(pair.first.second);
		}
	}
	triangles.clear();
	...
	for (const auto &i : odds)
	{
		...
		if (edge_info[make_pair(x, y)].size() > 2)
		{
			...
			for (const auto &it1 : p)
			{
				...
				if (glm::dot(nvertices[triangles[it1.first][(it1.second + 1) % 3]] - nvertices[i], dir) < 0)
				{
					triangles[it1.first][it1.second] = static_cast<int>(nvertices.size());
				}
			}
...
// Helper method to check if the structure is manifold
int Manifold::is_manifold()
{
...
// Save an OBJ representation of the manifold to a file
void Manifold::SaveOBJ(const char *filename)
{
...
// Save the manifold (supports color output)
void Manifold::Save(const char *filename, bool color)
{
	std::ofstream os(filename);
	if (color)
		os << "COFF\n";
